//
//  SignUpVipPresenter.swift
//  IChatVIP
//
//  Created by Александр Бадмаев on 28.03.2021.
//  Copyright (c) 2021 ___ORGANIZATIONNAME___. All rights reserved.
//

import UIKit

protocol SignUpVipPresentationLogic {
    func presentData(response: SignUpVip.Model.Response.ResponseType)
}

class SignUpVipPresenter: SignUpVipPresentationLogic {
    weak var viewController: SignUpVipDisplayLogic?
    
    func presentData(response: SignUpVip.Model.Response.ResponseType) {
        switch response {
        case let .presentAlert(title, message, type):
            viewController?.displayData(viewModel: .displayAlert(title: title, message: message, type: type))
        }
    }
    
}
