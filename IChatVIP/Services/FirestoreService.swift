...
class FirestoreService {
    ...
    private func getWaitingChatMessages(chat: MChat, completion: @escaping (Result<[MMessage], Error>) -> Void) {
        ...
        // В отличии от Listener запрос уходит один раз
    ...
    func changeToActive(chat: MChat, completion: @escaping (Result<Void, Error>) -> Void) {
        // 1. Берем все сообщения из ожидающего чата
        // 2. Удаляем ожидающий чат
        // 3. Создаем активный чат с сообщениями
        getWaitingChatMessages(chat: chat) { (result) in
            switch result {
            case .success(let messages):
    ...
    func sendMessage(chat: MChat, message: MMessage, completion: @escaping (Result<Void, Error>) -> Void) {
        //1. Создаем активный чат у друга, если он еще не создан
        //2. Добавляем другу в этот активный чат новое сообщение
        //3. Это новое сообщение добавляем и себе
        let friendRef = usersRef.document(chat.friendId).collection("activeChats").document(currentUser.id)
        let friendMesssageRef = friendRef.collection("messages")
        ...
        friendRef.setData(chatForFriend.representation) { (error) in
            ...
            friendMesssageRef.addDocument(data: message.representation) { (error) in
                if let error = error {
                    completion(.failure(error))
                    return
                }
